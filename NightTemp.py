...
class NightTemp():
    ...
    def getHighTemp(self):
        ...
        print(rows)
        print("Highest Night temp : {0} on {1}".format(rows[len(rows)-1][1],rows[len(rows)-1][0]))
    def getLowTemp(self):
        rows = []
        with open(file,'r') as csvfile:
            reader = csv.reader(csvfile,delimiter =',')
            ...
            for i in range(n):
                for j in range(0,n-i-1):
                    if(int(rows[j][1]) > int(rows[j+1][1])):
                        rows[j] , rows[j+1] = rows[j+1] , rows[j]
        print("Lowest Night Temp : {0} on {1}".format(rows[0][1],rows[0][0]))
    def getAvgTemp(self):
        rows = []
        temperatures = []
        with open(file,'r') as csvfile:
            ...
            print("Average night temperature :{0}".format(avg))